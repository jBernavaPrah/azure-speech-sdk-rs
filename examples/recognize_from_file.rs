...
async fn main() -> azure_speech::Result<()> {
    ...
    let audio_stream = create_stream_audio("tests/audios/examples_sample_files_turn_on_the_lamp.mp3").await;
...
async fn create_stream_audio(path: impl AsRef<Path>) -> impl Stream<Item=Vec<u8>> {
